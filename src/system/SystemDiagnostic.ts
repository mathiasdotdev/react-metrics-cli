...
export class SystemDiagnostic {
  ...
  static generateContextualHelp(status: SystemStatus): string {
    ...
    if (!status.nexusConfigured) {
      ...
      message += chalk.cyan('   Les credentials seront demandés automatiquement lors de la première utilisation\n')
      message += chalk.cyan('   Ils seront chiffrés et stockés de manière sécurisée\n')
      message += chalk.cyan('   Lancez: ') + chalk.white('react-metrics analyze') + chalk.cyan(' ou ') + chalk.white('react-metrics download\n\n')
    ...
    if (!status.binaryAvailable) {
      message += chalk.yellow('💡 Téléchargez le binaire avec: ') + chalk.white('react-metrics download\n')
    ...
    if (!status.reactMetricsConfigured) {
      message += chalk.yellow('💡 Créez une configuration personnalisée avec: ') + chalk.white('react-metrics config --init\n')
